...
class SimpleTradingBot:
    ...
    async def _perform_technical_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            technical_analysis = {
                ...
                'momentum': self._analyze_momentum(df),  # 这是一个字典
                'volume': self._analyze_volume_debug(df),
            ...
            # 注意：不要设置 technical_analysis['momentum'] = latest.get('Momentum', 0)
            # 因为momentum已经是一个字典了
            technical_analysis['momentum_value'] = latest.get('Momentum', 0)  # 使用不同的键名
    ...
    def _integrate_analyses_trend_first(self, game_theory: Dict, technical: Dict, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 1. 获取技术分析得分（这是关键！）
            tech_signal_strength = float(technical.get('signal_strength', 0))  # 这个是技术分析总分！
            ...
            adx = float(technical.get('adx', 20))
            ...
            if trend_direction == 'UP':
                trend_score = trend_strength * 5  # 最多10分
                integrated['signals'].append(f"上升趋势(强度:{trend_strength:.1f})")
            elif trend_direction == 'DOWN':
                trend_score = -trend_strength * 5
                integrated['signals'].append(f"下降趋势(强度:{trend_strength:.1f})")
            else:
                integrated['signals'].append("横盘整理")

            # 博弈论得分（-10到10）
            game_score = 0
            if whale_intent == 'ACCUMULATION':
                game_score = 6 * whale_confidence
                integrated['signals'].append(f"庄家吸筹({whale_confidence:.1%})")
            elif whale_intent == 'DISTRIBUTION':
                game_score = -6 * whale_confidence
                integrated['signals'].append(f"庄家派发({whale_confidence:.1%})")
            elif whale_intent == 'MANIPULATION_UP':
            ...
            if adx > 25:  # 强趋势市场
                ...
                integrated['strategy_type'] = 'TECHNICAL_DRIVEN'

            # ========== 计算综合得分 ==========
            # 归一化各项得分到相同尺度
            normalized_trend = trend_score / 10  # -1到1
            normalized_tech = tech_signal_strength / 5  # 假设技术得分范围是-5到5
            normalized_game = game_score / 10  # -1到1

            # 加权计算
            weighted_score = (
                    normalized_trend * weights['trend'] +
                    normalized_tech * weights['technical'] +
                    normalized_game * weights['game_theory']
            )
            ...
            signal_directions = []
            if trend_score != 0:
                signal_directions.append(1 if trend_score > 0 else -1)
            if tech_signal_strength != 0:
                signal_directions.append(1 if tech_signal_strength > 0 else -1)
            if game_score != 0:
                signal_directions.append(1 if game_score > 0 else -1)

            # 如果所有信号同向，加成20%
            if len(signal_directions) >= 2 and all(s == signal_directions[0] for s in signal_directions):
                final_score *= 1.2
                integrated['signals'].append("多信号共振")
            ...
            if final_score >= 2.0:
                ...
                if final_score >= 3.0:
                    ...
                    integrated['confidence'] = min(abs(final_score) / 10, 0.95)
            ...
            # ========== 输出详细评分 ==========
            print_colored(f"\n    📊 综合评分系统（修正版）", Colors.CYAN + Colors.BOLD)
            print_colored(
                f"    • 趋势得分: {trend_score:.1f} → 归一化: {normalized_trend:.2f} → 加权: {normalized_trend * weights['trend']:.2f} ({weights['trend']:.0%})",
                Colors.INFO)
            print_colored(
                f"    • 技术得分: {tech_signal_strength:.1f} → 归一化: {normalized_tech:.2f} → 加权: {normalized_tech * weights['technical']:.2f} ({weights['technical']:.0%})",
                Colors.INFO)
            print_colored(
                f"    • 博弈得分: {game_score:.1f} → 归一化: {normalized_game:.2f} → 加权: {normalized_game * weights['game_theory']:.2f} ({weights['game_theory']:.0%})",
                Colors.INFO)
